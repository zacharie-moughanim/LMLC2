...
Proof. 
  ...
  - simpl. intros n. destruct n.
    + intros H. apply Nat.lt_0_succ.
    + intros H. apply IHtl in H. apply le_n_S. apply H.
Qed.

Lemma find_opt_length_2'' : forall {X : Type} (l : list X) (n : nat), find_opt l n <> None -> n <= length l.
Proof. intros. apply Nat.lt_le_incl. apply find_opt_length_2'''. apply H. Qed.

Lemma find_opt_length_3 : forall {X : Type} (l : list X) (n : nat), n < length l -> find_opt l n <> None.
Proof. intros. unfold not. generalize dependent n. induction l as [|h t].
  - intros. simpl in H. inversion H.
  - intros *. intro H. simpl. simpl in H. destruct n as [|n'].
    + intro G. inversion G.
    + apply IHt. apply Lt.lt_S_n. apply H.
Qed.

Lemma find_opt_S : forall {X : Type} (l : list X) (n : nat), find_opt l (S n) <> None -> find_opt l n <> None.
Proof. intros. apply find_opt_length_2'' in H. Search find_opt length. apply find_opt_length_3. unfold lt. apply H.
Qed.
...
Lemma in_list_app3 : forall (x : nat) (l1 l2 : list nat), in_list l1 x = true -> in_list (l1++l2) x = true.
Proof. intro. induction l1 as [|h1 t1].
  - simpl. intros. discriminate H.
  - simpl. intros. destruct (x =? h1).
    + reflexivity.
    + apply IHt1. apply H.
Qed.

Lemma in_list_neq : forall (x y : nat) (l1 l2 : list nat), in_list (l1++y::l2) x = true -> x <> y -> in_list (l1++l2) x = true.
Proof. intros. induction l1 as [|h1 t1].
  - simpl. simpl in H. apply Nat.eqb_neq in H0. rewrite H0 in H. apply H.
  - simpl. simpl in H. destruct (x =? h1).
    + reflexivity.
    + apply IHt1. apply H.
Qed.

Lemma in_list_app_comm : forall (x : nat) (l1 l2 : list nat), in_list (l1++l2) x = true <-> in_list (l2++l1) x = true.
Proof. assert (one_dir : forall (x : nat) (l1 l2 : list nat), in_list (l1++l2) x = true -> in_list (l2++l1) x = true).
      { intros *. intro H. generalize dependent l1. induction l2 as [|h2 t2 IHt2].
        + intros. rewrite app_nil_r in H. apply H.
        + intros. simpl. destruct (x =? h2) eqn:eqxh2.
          * reflexivity.
          * apply IHt2. apply in_list_neq with (y := h2).
            -- apply H.
            -- apply Nat.eqb_neq. apply eqxh2.
      }
      intros. split.
  - apply one_dir.
  - apply one_dir.
...
Lemma not_in_neq : forall (x : nat) (l : list nat), in_list l x = false <-> (forall y, in_list l y = true -> y <> x).
Proof. intros. split.
  - intro H. induction l as [|h t].
    + intros. inversion H0.
    + intros. simpl in H0. destruct (y =? h) eqn:eqyh.
      * apply Nat.eqb_eq in eqyh. intro G. rewrite <- G in H. rewrite eqyh in H. simpl in H.
        rewrite Nat.eqb_refl in H. discriminate H.
      * apply IHt. simpl in H. destruct (x =? h).
        -- discriminate H.
        -- apply H.
        -- apply H0.
  - intro H. induction l as [|h t].
    + reflexivity.
    + simpl. destruct (x =? h) eqn:eqxh.
      * apply Nat.eqb_eq in eqxh. exfalso. symmetry in eqxh. generalize dependent eqxh. fold (not (x = h)). apply H.
